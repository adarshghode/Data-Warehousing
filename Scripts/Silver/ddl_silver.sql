...
BEGIN
	...
	PRINT('Truncating Table COMPLETED')
	...
	cst_create_date
		...
		FROM 
			...
			WHERE flag_last = 1)t
	PRINT('Inserting Table COMPLETED')
	------------------------------------------------------------------
	PRINT('========================================================')
	...
	prd_end_dt
		...
		prd_nm,
		ISNULL(prd_cost, 0) AS prd_cost, -- Null values replaced with zero
	...
	CASE WHEN UPPER(TRIM(CNTRY)) IN ('US', 'USA') THEN 'United States'
		 ...
		 WHEN cntry = '' OR CNTRY = NULL THEN 'N/A'
		 ELSE TRIM(CNTRY) 
	END CNTRY
